...
def lambda_handler(event, context):
    ...
    return {
        'statusCode': 200,
        'body': json.dumps('Success: Below is the cost breakdown for AmazonCloudWatch and AmazonVPC')
    }
...
def calculate_cumulative_costs(cost_and_usage):
    """
    Calculate the cumulative costs of Amazon CloudWatch, Amazon VPC, and Amazon DynamoDB for each resource.
    """
    cumulative_costs = defaultdict(float)
    for entry in cost_and_usage:
        product_name = entry.get('product/servicecode')
        if product_name in ['AmazonCloudWatch', 'AmazonVPC', 'AmazonDynamoDB']:
            ...
            cumulative_costs[(product_name, resource_name)] += unblended_cost
...
for (service, resource_name), cost in cumulative_costs.items():
    print(f"Service: {service}, Resource Name: {resource_name}, Cumulative Cost: ${cost:.6f}")
